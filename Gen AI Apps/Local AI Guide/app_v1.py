...
# Ensure you run: export GROQ_API_KEY="your-key" in your terminal first
try:
    client = Groq()
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Hi! I're your AI food guide. I can filter by cuisine or search radius. Try saying: *'Find me pizza places within 3km'*"}
    ]
            ...
            if tool_calls:
                for tool_call in tool_calls:
                    if tool_call.function.name == "fetch_osm_restaurants":
                        # Unpack argument strings cleanly into Python dictionary objects
                        args = json.loads(tool_call.function.arguments)
                        radius = args.get("radius", 1500)
                        cuisine = args.get("cuisine", None)
                        
                        # Show visual loading indicator to the user
                        ...
                        st.session_state.map_data = places  # Cache findings globally to force re-pinning
                        
                        # Compile context context string to pass back to LLM for final delivery
                        places_context = "\n".join([f"- {p['name']} (Cuisine: {p['cuisine']})" for p in places])
                        system_summary_prompt = f"The system found these physical locations matching the criteria:\n{places_context}\n\nPresent these options to the user nicely and let them know the points are updated on their map interface."
                        ...
                        # Rerun app instantly to forces the map visualization component to track the newly loaded markers
                        st.rerun()
            else:
                # Direct conversational text fallback if no location processing was invoked
...
with col2:
    ...
    # Initialize baseline Leaflet layout using open-source OpenStreetMap base-tiles
    m = folium.Map(location=[USER_LAT, USER_LON], zoom_start=14, tiles="OpenStreetMap")
    
    # Native User Position Marker
    folium.Marker(
        [USER_LAT, USER_LON], 
    ...
    # Dynamically draw pins out of cached session query data arrays
    ...
    # Render map component within Streamlit container
